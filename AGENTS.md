...
**Playwright E2E 测试**：`.Codex/test-settings.mjs` 包含完整的 Playwright E2E 测试脚本，使用 Chromium + dev auth 注入，可测试 profile/settings 等受保护页面。手动运行该脚本需要先启动 `pnpm dev`，然后 `node .Codex/test-settings.mjs`。
