// Chat Pipeline — 核心对话编排逻辑
// 从 src/app/api/ai/chat/route.ts 提取，减少路由文件体积

import { NextResponse } from 'next/server';
import { callDeepSeek, callDoubaoChat, callQwen, callOpenRouter, generateImage, submitVideoTask, callDoubaoVision, fetchWeather } from '@/lib/ai-services';
...
import { detectIntent, buildPrompt, GEN_JSON_TEMPLATE } from '@/lib/assistant';
...
import type { WeatherData } from '@/lib/ai-services';
...
export function buildSystemPrompt(
  ...
  const summaryBlock = historySummary
    ...
    : '';
  return [summaryBlock, memoryBlock, knowledgeBlock, skillsBlock, baseSystemPrompt]
    .filter(Boolean)
    .join('\n\n---\n\n');
}

// ====== RAG 上下文组装（Memory + Knowledge） ======
export async function assembleContext(
  ...
  memoryBlock: string;
  knowledgeBlock: string;
  contextStats: { memoriesUsed: number; inspirationsUsed: number; knowledgeUsed: number; webSearchUsed: boolean; skillsMatched: number } | null;
}> {
  let memoryBlock = '';
  let knowledgeBlock = '';
  ...
  try {
    ...
    if (ctxEmbedding.length > 0) {
      ...
      await builtinMem.initialize(userId);
      memMgr.addProvider(builtinMem);

      const kMgr = new KnowledgeManager();
      kMgr.addProvider(new InspirationKnowledgeProvider(userId));
      ...
      const [mBlock, kResult] = await Promise.all([
        ...
        kMgr.search(effectiveContent, ctxEmbedding, userId),
      ]);
  ...
  return { memoryBlock, knowledgeBlock, contextStats };
}
...
export type ChatPipelineResult =
  | { type: 'stream'; response: NextResponse }
  | {
      type: 'json';
      analysis: any;
      modelUsed: string;
...
export async function executeChatPipeline(input: ChatPipelineInput): Promise<ChatPipelineResult> {
  const { content, images, videos, documents, searchResults, sessionId: session_id, selectedModel, useStream, userId, requestUrl } = input;
  ...
  const linkContext: LinkContext | null = isLink
    ? await analyzeLink(requestUrl, normalizeUrl(content))
    : null;

  // ====== 文档附件抽取 ======
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const documentContext: string[] = hasDocuments && supabaseUrl
    ? await extractDocuments(documents, userId, supabaseUrl)
    : [];
  ...
  // ====== V2.0: 多源上下文检索（记忆 + 知识库） ======
  const { memoryBlock, knowledgeBlock, contextStats: baseContextStats } = await assembleContext(effectiveContent, userId);
  let contextStats = baseContextStats;
  ...
  const systemPrompt = buildSystemPrompt(baseSystemPrompt, historySummary, memoryBlock, knowledgeBlock, skillsBlock);
...
请直接以自然语言输出，不需要JSON格式。`;
  ...
  if (useStream && !isMultimodal) {
    const streamResponse = createChatStreamResponse(
      userPrompt,
      genMaxTokens,
      session_id,
      userId,
    ...
    return { type: 'stream', response: streamResponse };
  }

  // ====== 3. 调用 AI 模型（非 streaming） ======
...
JSON 格式：
{
  ...
  const linkFetchFailed = !!(linkContext && (!linkContext.extractedContent || linkContext.extractedContent.trim().length < 200));
