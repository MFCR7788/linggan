...
- [ ] 视频已生成并自动保存到灵感库`,
  ...
  },
  {
    ...
    displayName: "💡 今日创作提案",
    description:
      "基于热点趋势、账号类型和创作偏好，智能生成今日内容选题提案。每个提案包含选题角度、推荐生成流程和预填参数。用户选择后自动串联调用生成工具完成全流程创作。适合：不知道今天拍什么、需要选题灵感、想蹭热点但不知道怎么切入等场景。",
    category: "内容创作",
    tags: [
      "选题", "提案", "灵感", "热点", "今日",
      "创作方向", "内容规划", "选题建议",
    ...
    promptTemplate: `你正在执行「今日创作提案」任务。

**功能说明**: 基于实时热点、用户账号类型和历史创作偏好，智能生成个性化内容选题。每个提案自带推荐工具链和预填参数，选中后自动全流程生成。
...
**执行步骤**:
1. 调用 suggest_content_ideas 工具生成提案
2. 提案以 <choices> 卡片展示（每张卡片包含选题+一句话说明+推荐流程）
3. 用户选择后，按提案的 suggested_pipeline 依次调用生成工具
4. 生成完成后自动保存到灵感库

**质量标准**:
- 提案差异化：3个提案覆盖不同角度（不是同一选题的变体）
- 热点关联：每个提案说明为什么现在做合适
- 流程实用：推荐 1-3 步工具链，不给复杂或不可用的流程

**自检清单**（返回前逐项确认）:
- [ ] 3个提案角度不同
- [ ] 每个提案有 suggested_pipeline
- [ ] 用户选择后已按 pipeline 执行
- [ ] 生成内容已自动保存到灵感库`,
    boundTools: ["suggest_content_ideas"],
    version: "1.0.0",
    visibility: "official",
    installCount: 0,
    createdAt: "2026-06-13T00:00:00Z",
