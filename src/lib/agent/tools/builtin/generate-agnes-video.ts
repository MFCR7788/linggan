...
- 281≈11.7s / 321≈13.4s / 361≈15s(~60字) / 441≈18.4s(~80字)`,
  ...
  async handler(params, ctx) {
    ...
    try {
      ...
      let mergedUrl = segmentUrls[0]; // fallback
      if (mergedPath && mergedPath !== segmentUrls[0]) {
        // TODO: 上传到 Supabase Storage 获取 URL
        // 暂时返回段列表，让调用方处理
      }
      ...
      return {
        success: true,
        output: [
          `已生成 ${segments.length} 段口播视频（总长约 ${totalDuration.toFixed(0)} 秒）✨`,
          '',
