  ...
  const handleExpand = async (item: HotspotItem) => {
    if (expandedId === item.id) {
      ...
      if (!item.is_read) {
        ...
        // 后台同步 API
        syncDevAuthCookie();
        fetch(`/api/hotspot/${item.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', ...authHeaders }, body: JSON.stringify({ is_read: true }) }).catch((e) => console.error('[hotspot] 标记已读失败:', e));
      }
    }
  };
...
export default function HotspotPageContent() {
  return (
    <ProtectedRoute>
