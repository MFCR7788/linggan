...
const features: FeatureEntry[] = [
  ...
  },
  {
    icon: <Wallet size={20} />,
    title: '灵感点 + 加油包 + 订阅',
    desc: 'V2.0.3 全新计费系统:统一「灵感点」credits 计量,加油包(一次性) + 订阅(月度赠送)双轨。',
    color: '#F472B6',
    highlights: [
      '灵感点(credits)是灵集唯一计费单位,所有 AI 功能按调用消耗',
      '加油包(4 档):体验包 ¥29/120 credits、标准包 ¥119/650(省 20%)、大包 ¥399/2800(省 30%)、企业包 ¥1599/15000(省 45%)',
      '加油包余额长期有效(180-365 天),用完再买,不订阅也能用',
      '订阅(5 档):免费版 ¥0/30 credits、个人版 ¥29/150、创作者版 ¥99/500、Studio ¥299/1800、企业版 ¥999/6000',
      '订阅 credits 每月发放,月底未消耗完会清零(加油包余额不受影响)',
      '先扣加油包再扣订阅(加油包优先,优先用快过期的部分)',
      '余额不足时所有 AI 调用统一返 402 错误,弹「去充值」引导',
      '微信 H5 支付已上线:加油包/订阅在「我的灵感点」选档位 → 微信扫码/拉起 → 支付完成自动到账(回调 + 5min 兜底轮询)',
    ],
...
const faqs: FAQEntry[] = [
  ...
  {
    q: '灵感点(credit)是什么?怎么用?',
    a: 'V2.0.3 起,灵集所有 AI 功能统一按「灵感点(credit)」扣费,1 credit ≈ ¥0.001(随档位浮动),透明清晰。\n\n扣费标准(参考价,以实际为准):\n- AI 文案:2 credits/条(批量 N 个变体 × N 倍)\n- AI 图片:1-2 credits/张(分模型)\n- AI 数字人:10-30 credits/条\n- AI 视频:3-30 credits/段(分档位,流畅档最便宜)\n- AI 配音:1 credit/百字\n\n获取方式 2 种:\n1. 加油包(一次性):¥29-1599 买 credits,长期有效(180-365 天),适合低频/不想订阅的用户\n2. 订阅(月度):¥29-999 每月自动送 credits,月底清零,适合稳定高频用户\n\n查看余额: 个人中心 → 我的灵感点 → 顶部显示当前余额 + 累计消耗 + 累计充值。\n\n最低余额预警: < 20 credits 时页面顶部黄色提示「余额不足」,建议充值。',
  },
  {
    q: '加油包和订阅有什么区别?选哪个划算?',
    a: '核心区别:\n- 加油包(一次性):¥29 起,credits 长期有效(180-365 天),用完再买,不订阅也能用\n- 订阅(月度):¥29-999/月,每月初送 credits,月底未用完会清零(加油包余额不受影响)\n\n谁适合加油包:\n- 用量不稳定(一个月可能 1 次,可能 10 次)\n- 不想被订阅绑住\n- 想长期囤 credits 等大活动\n\n谁适合订阅:\n- 每月稳定使用 5 次以上 AI 视频/数字人\n- 想获得月度赠送的稳定感(¥29 个人版 150 credits ≈ 5-10 次生成)\n- 想解锁创作者版/Studio 版的专属功能(声音克隆 1 次/月、数字分身 5 次/月等)\n\n可以叠加: 同时买加油包 + 订阅,加油包余额优先消耗(不会和订阅赠送混算)。\n\n价格对比(单 credit):\n- 加油包:体验包 ¥0.242、标准包 ¥0.183、大包 ¥0.142、企业包 ¥0.107\n- 订阅:免费版 -、个人版 ¥0.193、创作者版 ¥0.198、Studio ¥0.166、企业版 ¥0.167\n\n单看单价 Studio/企业版最划算(¥0.166-0.167),但要分摊到「每月用完」。\n\n推荐组合: 订阅 Studio(¥299)保证月度基础用量 + 大加油包(¥399)做余量,媒体运营官的甜点配置。',
  },
  {
    q: '余额不足会怎样?会被中断吗?',
    a: 'V2.0.3 起,所有 AI 调用在生成前会先校验余额(原子扣点,不会扣一半)。\n\n余额不足的处理:\n1. 立即返 402 错误 + 明确文案「余额不足:需要 X credits,当前 Y credits」\n2. 前端弹「去充值」按钮,直接跳到加油包页\n3. 已生成内容不受影响(已扣的不会退,但失败的会全额退)\n4. 余额 < 20 时页面顶部黄色横幅提示,不会突然中断\n\n并发安全: 后端用 PostgreSQL RPC 函数 consume_credits_atomic 做 CAS 扣点,即使你同时点 10 个生成按钮,也不会出现「扣了一半余额失败」的脏数据。\n\n退款: AI 生成失败(网络/超时/内容审核)会自动退 credit,1-3 分钟内到账,在流水里能看到「退款」记录。',
  },
  {
    q: 'AI 视频多首尾帧模式怎么用?',
...
const guides: GuideSection[] = [
  ...
  },
  {
    ...
    color: '#F472B6',
    steps: [
      { step: 1, content: '入口 1: 个人中心 → 我的灵感点 → 加油包/订阅卡片 → 点「加油包」/「订阅套餐」' },
      { step: 2, content: '入口 2: AI 创作中心 → 余额不足弹窗 → 点「去充值」直接跳加油包页' },
      { step: 3, content: '加油包页(4 档):体验包(¥29) / 标准包(¥119) / 大包(¥399) / 企业包(¥1599),每个包有「主 credits + 赠送 credits」+ 单价 + 有效期' },
      ...
      { step: 6, content: '订阅升级: 个人中心 → 我的灵感点 → 订阅套餐 → 5 档可选(免费/个人/创作者/Studio/企业)→ 点「立即订阅」→ 弹确认窗(微信支付)→ 立即到账首月 credits + 30 天有效期,到期前自动续费(微信 H5 自动扣款)+ 赠送下月 credits' },
      { step: 7, content: '取消订阅:个人中心 → 我的灵感点 → 「我的订阅」section → 活跃订阅点「取消自动续费」→ 当前周期 credits 仍可用到本周期结束,周期结束后自动降级到免费版' },
    ],
  },
];
