...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const script = [
      ...
      `rm -f "${tarPath}"`,
      '',
      '# Copy old node_modules as base for faster npm install',
      'if [ -d /opt/lingji/node_modules ]; then',
      '  echo "Copying node_modules from old deployment..."',
      '  cp -a /opt/lingji/node_modules /opt/deploy-pkg/node_modules',
      'fi',
      '',
      `if [ -f "${ENV_FILE}" ]; then`,
      ...
      '  echo "npm install failed, trying without old node_modules..."',
      '  rm -rf node_modules',
