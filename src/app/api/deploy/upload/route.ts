...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const script = [
      ...
      `mkdir -p /opt/deploy-pkg`,
      `if ! tar xzf "${tarPath}" --strip-components=1 -C /opt/deploy-pkg 2>/dev/null; then`,
      ...
      '# Restart',
      'cd /opt/lingji',
      'pm2 restart lingji 2>/dev/null || pm2 start node_modules/next/dist/bin/next --name lingji -- start -p 3000',
      'pm2 save',
      'echo "=== Deploy OK $(date) ==="',
      'pm2 list',
    ].join('\n');
